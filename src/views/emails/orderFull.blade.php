...
@section('content')
    ...
    <table lang="ru" style="width: 100%; padding-top: 15px" cellspacing="0" cellpadding="5">
        <thead>
        <tr>
            <td style="border: #bcbcbc 1px solid;font:14px/16px Calibri,Helvetica,Arial,sans-serif;font-weight:700"></td>
                ...
                <td style="border: #bcbcbc 1px solid;font:14px/16px Calibri,Helvetica,Arial,sans-serif;">
                        @if(isset($item->catalog->full_url))
                            <a href="{!! $item->catalog->full_url !!}">
                                <img class="all-width" src="{{ env('APP_URL') }}{!! $item->catalog->getFirstImage->getUrl('140x140') !!}">
                            </a>
                        @endif
                    </td>
                @if(isset($item->catalog->full_url))
        ...
        @else
            <tr>
                <td style="border: #bcbcbc 1px solid;font:14px/16px Calibri,Helvetica,Arial,sans-serif;text-align: right" colspan="5"><strong>Всего к оплате: {!! $data['cost'] !!} руб.</strong></td>
            </tr>
        @endif
        </tbody>
    </table>
